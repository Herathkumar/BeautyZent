(function () {
  ...
  function openBookOnline() {
    window.open(BOOK_ONLINE_URL, "_blank", "noopener,noreferrer");
  }

  function offerBookingFollowUp() {
    addChips([
      {
        label: copy.bookOnline,
        primary: true,
        onClick: () => {
          addMessage(copy.bookOnline, "user");
          clearQuick();
          botSay("Opening online booking for you — pick a service, stylist, and time.");
          openBookOnline();
        },
      },
      {
    ...
    addChips([
      {
        ...
        onClick: () => {
          addMessage(copy.bookOnline, "user");
          clearQuick();
          botSay("Great — opening the booking page. See you at the salon!");
          openBookOnline();
        },
      },
      {
        label: copy.requestCallback,
        onClick: () => {
          addMessage(copy.requestCallback, "user");
          clearQuick();
          state.step = "name";
          botSay(copy.askName);
        },
